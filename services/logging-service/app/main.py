...
def set_consent(user_id):
    ...
    if record is None:
        record = Consent(user_id=user_id, granted=data["granted"])
...
def get_logs(user_id):
    ...
    return jsonify({
        "items": [{"id": l.id, "user_id": l.user_id, "game_id": l.game_id, "action": l.action, "message": l.message, "created_at": l.created_at.isoformat()} for l in logs],
        "total": len(logs)
    })
